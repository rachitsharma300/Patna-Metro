...
// ForwardRef is used to allow Bot.jsx to call triggerSearch from this component
const RouteFinder = forwardRef(
  ({ source, destination, setSource, setDestination, triggerSearch, setTriggerSearch }, ref) => {
    const { t } = useTranslation();
    const [stations, setStations] = useState([]); // List of all stations
    const [selected, setSelected] = useState({ source: source || "", destination: destination || "" }); // Selected source/destination
    ...
    // Expose the triggerSearch method to parent (like Bot.jsx)
    useImperativeHandle(ref, () => ({
      triggerSearch: () => getRoute(),
    }));
    ...
    // Auto-trigger route search when triggerSearch flag is set to true
    useEffect(() => {
      if (triggerSearch && selected.source && selected.destination) {
        getRoute();
        setTriggerSearch(false); // Reset trigger flag
      }
