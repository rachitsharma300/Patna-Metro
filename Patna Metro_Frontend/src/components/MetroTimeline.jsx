  ...
  return (
    <div className="relative">
      ...
      {events.map((event, index) => (
        <div key={index} className={`relative mb-8 ${index % 2 === 0 ? 'pr-8 text-right' : 'pl-8 text-left'}`}>
          <div className={`absolute top-0 w-4 h-4 rounded-full bg-blue-600 ${index % 2 === 0 ? '-right-2' : '-left-2'}`}></div>
          <div className={`inline-block p-4 rounded-lg shadow-sm border border-gray-200 ${index % 2 === 0 ? 'bg-blue-50' : 'bg-white'}`}>
